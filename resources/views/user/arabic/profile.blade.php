...
@section('content')
    <div class="container-fluid row">
        ...
        <div class="col-md-2 col-xs-3 pull-left" style="margin-bottom: 20px">
            ...
            <a href="/user/evaluations/{{$user->id}}"><p style="text-align: left; font-family: 'Segoe UI'; margin-left: 30px; color: #0275d8"><span class="pull-left">تقييم</span>&nbsp;{{$user->no_of_likes + $user->no_of_dislikes}}</p></a>
        </div>
        ...
        <div class="col-md-1 col-md-offset-1 col-xs-2 col-xs-offset-1 pull-right" style="margin-bottom: 20px">
            @if($user->followers != null)
                <form method="post" action="{{in_array(Auth::user()->id, $user->followers) ? '/user/unfollow/'.$user->id : '/user/follow/'.$user->id}}" style="width: 80px">
                    @csrf
                    <input class="btn btn-default btn-sm pull-right follow_button" type="submit" value="{{in_array(Auth::user()->id, $user->followers) ? 'إلغاء' : 'متابعة'}}"><br>
                    <p style="text-align: left; font-family: 'Segoe UI'; margin-right: 35px; color: darkgray; font-size: 14px"><span class="pull-right">{{count($array)-1}}</span>متابع&nbsp;</p>
                </form>
            @else
                @if(Auth::user()->id != $user->id)
                    <form method="post" action="/user/follow/{{$user->id}}" style="width: 80px">
                        @csrf
                        <input class="btn btn-default btn-sm pull-right follow_button" type="submit" value="متابعة"><br>
                        <p style="text-align: left; font-family: 'Segoe UI'; margin-right: 35px; color: darkgray; font-size: 14px"><span class="pull-right">0</span>متابع&nbsp;</p>
                    </form>
                @else
        ...
        <div class="col-md-12 col-xs-12">
            ...
            @if($user->id !== Auth::user()->id)
                <div class="col-md-6 col-xs-6 pull-left" style="margin-bottom: 60px; text-align: center; font-size: 18px; font-family: 'Segoe UI'; color:#0275d8">
                    <a style="text-decoration: none; margin-right: 8%" data-toggle="modal" data-target="#myModal" id="open">  محادثة&nbsp;<i class='fa fa-comments'></i></a>
                </div>
            @endif
            <!-- Modal -->
        ...
        @if(count($posts) > 0)
            ...
            {{--<div class="col-md-12 col-xs-12" style="text-align: center; font-size: 25px; font-family: 'Segoe UI'; margin-bottom: 60px; color: darkgray">لا توجد معروضات أخرى</div>--}}
        @else
            <div class="col-md-12 col-xs-12" style="text-align: center; font-size: 25px; font-family: 'Segoe UI'; margin-bottom: 60px; color: darkgray">لا توجد معروضات</div>
        @endif
    </div>
@endsection