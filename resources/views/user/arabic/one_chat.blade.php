    ...
    <div class="container-fluid row">
        ...
        @if(count($messages) > 0)
            <div class="col-md-6 col-md-offset-3 col-xs-10 col-xs-offset-1" style="margin-bottom: 30%; background-color: whitesmoke">
                @if($chat_owner->id == Auth::user()->id)
                    <div class="col-md-12 col-xs-12" style="text-align: center"><br>
                        <a href="/user/profile/{{$chat_owner->id}}" style="color: #0275d8; font-size: 20px">{{$post_owner->name}}</a><br><br>
                    </div>
                @else
                    <div class="col-md-12 col-xs-12" style="text-align: center"><br>
                        <a href="/user/profile/{{$chat_owner->id}}" style="color: #0275d8; font-size: 20px">{{$chat_owner->name}}</a><br><br>
                    </div>
                @endif
                @foreach($messages as $message)
                    @if($message->sender_id == Auth::user()->id)
                        <div class="col-md-1 col-xs-1" style="background-color: whitesmoke">
                            @if(Auth::user()->profile_photo != null)
                                <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/'.Auth::user()->profile_photo)}}">
                            @else
                                <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/default_profile.jpg')}}">
                            @endif
                        </div>
                        <div class="col-md-7 col-xs-7" style="background-color: whitesmoke; text-align: left">
                            <div style="background-color: #5cb85c; height: auto; border-radius: 5px; min-height: 30px">
                                <p style="font-size: 14px; margin-left: 10px; color: white">{{$message->body}}</p>
                    ...
                    @else
                        @if($post_owner-> id != Auth::user()->id)
                            <div class="col-md-1 col-xs-1 pull-right" style="background-color: whitesmoke">
                                @if($post_owner->profile_photo != null)
                                    <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/'.$post_owner->profile_photo)}}">
                                @else
                                    <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/default_profile.jpg')}}">
                                @endif
                            </div>
                        ...
                        @if($chat_owner-> id != Auth::user()->id)
                            <div class="col-md-1 col-xs-1 pull-right" style="background-color: whitesmoke">
                                @if($chat_owner->profile_photo != null)
                                    <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/'.$chat_owner->profile_photo)}}">
                                @else
                                    <img class="img-circle" style="border-radius: 50%; width: 40px; height: 40px" src="{{asset('images/profiles/default_profile.jpg')}}">
                                @endif
                            </div>
                        @endif
                        <div class="col-md-7 col-xs-7 pull-right" style="background-color: whitesmoke; text-align: right">
                    ...
                    @endif
                @endforeach
                <form action="/new_message_reply/{{$message->chat_room}}/{{$message->sender_id}}" method="post" class="row">
                    @csrf
                    <textarea class="col-md-11 col-xs-11 placeholder-styling" name="body" style="background-color: white; height: 35px; text-align: right; margin-bottom: 10px" placeholder="أكتب ردك هنا" required></textarea>
